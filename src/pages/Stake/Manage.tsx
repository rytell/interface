...
export default function Manage({
  ...
  const userRadiUnstaked = useTokenBalance(account ?? undefined, stakingInfo?.stakedAmount?.token)
  const showGetRadiButton = useMemo(() => {
    if (!userRadiUnstaked || !stakingInfo) return true
    return Boolean(stakingInfo?.stakedAmount?.equalTo('0') && userRadiUnstaked?.equalTo('0'))
  }, [stakingInfo, userRadiUnstaked])
  ...
  return (
    <PageWrapper gap="lg" justify="center">
      <RowBetween style={{ gap: '24px' }}>
        <CurrencyLogo currency={radi} />
        <TYPE.mediumHeader style={{ margin: 0 }}>{t('earnPage.radiStaking')}</TYPE.mediumHeader>
      ...
      {stakingInfo && (
        <>
          <StakingModalSingleSide
            ...
            userLiquidityUnstaked={userRadiUnstaked}
          />
      ...
      <PositionInfo gap="lg" justify="center" dim={showGetRadiButton}>
        <BottomSection gap="lg" justify="center">
          <StyledDataCard bgColor={backgroundColorStakingToken} showBackground={!showGetRadiButton}>
            <CardSection>
              <AutoColumn gap="md">
                <RowBetween>
